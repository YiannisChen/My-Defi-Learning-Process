...
describe("Pool State Test", function () {
    ...
    it("should have unlocked pool after initialization", async function () {
        // Check unlocked state after initialization (already done in before)
        let unlocked = await pool.unlocked();
        console.log("Pool unlocked after init:", unlocked);
        expect(unlocked).to.be.true;
        
        // Check that pool is properly initialized
        const sqrtPriceX96 = await pool.sqrtPriceX96();
        expect(sqrtPriceX96).to.equal("79228162514264337593543950336");
    });
}); 