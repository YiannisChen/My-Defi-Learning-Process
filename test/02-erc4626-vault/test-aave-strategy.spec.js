...
async function main() {
    try {
        ...
        console.log(
            `DAI Balance: ${ethers.formatUnits(daiBalance, daiDecimals)} DAI`,
        );

        if (daiBalance.toString() === "0") {
            console.error(
                "❌ No DAI balance! Please get some DAI from the Aave faucet before testing.",
            );
            return;
        }
        ...
        try {
            ...
            console.log(
                `Current aDAI Balance: ${ethers.formatUnits(aTokenBalance, daiDecimals)} aDAI`,
            );
            console.log(
                "✅ Successfully connected to aDAI token at:",
                ADAI_ADDRESS,
            );
        } catch (error) {
        ...
        try {
            ...
            try {
                const setVaultTx = await aaveStrategy.setVault(
                    deployer.address,
                );
                ...
                console.log(
                    `Approving strategy to spend ${ethers.formatUnits(approvalAmount, daiDecimals)} DAI...`,
                );
                ...
                try {
                    ...
                    console.log(
                        `Attempting to deposit ${ethers.formatUnits(depositAmount, daiDecimals)} DAI...`,
                    );
                    ...
                    try {
                        ...
                        if (isPaused) {
                            console.log(
                                "⚠️ Strategy is paused. Attempting to unpause...",
                            );
                        ...
                        const depositTx = await aaveStrategy.deposit(
                            depositAmount,
                            {
                        ...
                        console.log(
                            "✅ Deposit transaction successful! Hash:",
                            receipt.hash,
                        );
                        ...
                        try {
                            ...
                            console.log(
                                `Total value in strategy: ${ethers.formatUnits(totalValue, daiDecimals)} DAI`,
                            );

                            // Check total deposited amount tracked by the strategy
                            const totalDeposited =
                                await aaveStrategy.totalDeposited();
                            console.log(
                                `Total deposited tracked by strategy: ${ethers.formatUnits(totalDeposited, daiDecimals)} DAI`,
                            );

                            // Check aToken balance of strategy
                            const strategyATokenBalance =
                                await aToken.balanceOf(strategyAddress);
                            console.log(
                                `Strategy aDAI balance: ${ethers.formatUnits(strategyATokenBalance, daiDecimals)} aDAI`,
                            );

                            // Check any loose tokens not yet deposited
                            const looseTokens =
                                await dai.balanceOf(strategyAddress);
                            console.log(
                                `Loose DAI in strategy: ${ethers.formatUnits(looseTokens, daiDecimals)} DAI`,
                            );

                            // Try to get APY information
                            try {
                                const estimatedAPY =
                                    await aaveStrategy.estimatedAPY();
                                console.log(
                                    `Estimated APY: ${estimatedAPY.toString() / 100}%`,
                                );
                            } catch (error) {
                                console.log(
                                    "⚠️ Could not get estimated APY:",
                                    error.message,
                                );
                            }
                            ...
                            try {
                                ...
                                console.log(
                                    `Withdrawing ${ethers.formatUnits(withdrawAmount, daiDecimals)} DAI...`,
                                );

                                try {
                                    const withdrawTx =
                                        await aaveStrategy.withdraw(
                                            withdrawAmount,
                                            {
                                                gasLimit: 1000000,
                                            },
                                    ...
                                    console.log(
                                        "✅ Withdrawal successful! Hash:",
                                        withdrawReceipt.hash,
                                    );

                                    // Final checks
                                    console.log("\n--- Final State Check ---");
                                    const finalDaiBalance = await dai.balanceOf(
                                        deployer.address,
                                    );
                                    console.log(
                                        `Final DAI balance: ${ethers.formatUnits(finalDaiBalance, daiDecimals)} DAI`,
                                    );

                                    const finalValue =
                                        await aaveStrategy.totalValue();
                                    console.log(
                                        `Final value in strategy: ${ethers.formatUnits(finalValue, daiDecimals)} DAI`,
                                    );
                                    ...
                                    try {
                                        const withdrawAllTx =
                                            await aaveStrategy.withdrawAll({
                                                gasLimit: 1000000,
                                            });
                                        const withdrawAllReceipt =
                                            await withdrawAllTx.wait();
                                        console.log(
                                            "✅ WithdrawAll successful! Hash:",
                                            withdrawAllReceipt.hash,
                                        );

                                        const afterWithdrawAllValue =
                                            await aaveStrategy.totalValue();
                                        console.log(
                                            `Value in strategy after withdrawAll: ${ethers.formatUnits(afterWithdrawAllValue, daiDecimals)} DAI`,
                                        );

                                        const finalDaiBalanceAfterAll =
                                            await dai.balanceOf(
                                                deployer.address,
                                            );
                                        console.log(
                                            `Final DAI balance after withdrawAll: ${ethers.formatUnits(finalDaiBalanceAfterAll, daiDecimals)} DAI`,
                                        );
                            ...
                            console.error(
                                "❌ Failed to check post-deposit state:",
                                error,
                            );
                            console.log("Error message:", error.message);
                        }
                        ...
                        if (error.message.includes("execution reverted")) {
                            ...
                            try {
                                ...
                                console.log(
                                    "Testing direct Aave Pool interaction...",
                                );

                                // First approve the Aave Pool directly
                                const directApproveTx = await dai.approve(
                                    AAVE_POOL_ADDRESS,
                                    depositAmount,
                                );
                                await directApproveTx.wait();
                                console.log(
                                    "✅ Direct Aave Pool approval successful",
                                );
                                ...
                                try {
                                    ...
                                    // Encode the calldata
                                    const calldata =
                                        poolInterface.encodeFunctionData(
                                            "supply",
                                            [
                                                DAI_ADDRESS,
                                                depositAmount,
                                    ...
                                    console.log(
                                        "✅ Direct Aave supply worked! The issue is in the strategy contract.",
                                    );
                                } catch (supplyError) {
                                    console.error(
                                        "❌ Direct Aave supply failed:",
                                        supplyError.message,
                                    );
                                    console.log(
                                        "This suggests an issue with the Aave Pool interface, not your strategy.",
                                    );
                                    console.log(
                                        "The Aave V3 Pool on Sepolia might have a different interface than expected.",
                                    );
                                }
                            } catch (debugError) {
                                console.error(
